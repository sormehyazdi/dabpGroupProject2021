...
## Global variable
path = os.getcwd()
path = '/Users/Pooja/Downloads/dabpGroupProject2021-main 2/V2_DABP_Code'
...
def min_max_distance(pod_sites, blocks, distance, population, loadingSites, capacity, supplies, labor, cost, budget, opening_cost, bigM, max_days_open):
    ...
    Z = m2.addVar(lb=0.0, obj=1.0, vtype=GRB.CONTINUOUS)
    ...
    for i in pod_sites:
        ...
        for k in blocks:
            m2.addConstr((y2[i, k]*distance[k, i]) <= Z)
            m2.addConstr(y2[i,k] <= z2[i])
    ...
    return(m2.objVal, lst, pod_days, block_dist) 
...
def run_scenario_one():
    ...
    opening_cost = [5000, 10000, 25000]
    budget = [1000000, 2500000, 5000000]
...
def run_scenario_one_m2():
    ## Global epidemic in Allegheny county

    ## Setting up some parameters that will be looped through the main function
    #opening_cost = [2500, 5000, 10000, 15000]
    #budget = [1000000, 1500000, 2000000, 2500000]
    opening_cost = [2500, 5000, 10000]
    ...
    for b in budget:
        for op_co in opening_cost:
            ...
            ## List of Tuples #2
                m2_pods = [(b, op_co, pod[0], pod[1]) for pod in pod_days]
                #s1_m1_pods.extend(m1_pods)
                s1_m2_pods = s1_m2_pods + m2_pods
                m2_dist = [(b, op_co, bdist[0], bdist[1]) for bdist in block_dist]
                #s1_m1_dist.extend(m1_dist)
    ...
    return(s1_m2_opt_vals_df, s1_m2_pods_df, s1_m2_dist_df)
...
def run_scenario_two_m1():
    ...
    return(s2_m1_opt_vals_df, s2_m1_pods_df, s2_m1_dist_df)

def run_scenario_two_m2():
    ## Global epidemic in Allegheny county
    ...
    return(s2_m2_opt_vals_df, s2_m2_pods_df, s2_m2_dist_df)
...
def main():
    ...
    #(s1_m2_opt_vals_df, s1_m2_pods_df, s1_m2_dist_df) = run_scenario_one_m2()
    #(s2_m1_opt_vals_df, s2_m1_pods_df, s2_m1_dist_df) = run_scenario_two_m1()
    (s2_m2_opt_vals_df, s2_m2_pods_df, s2_m2_dist_df) = run_scenario_two_m2()
