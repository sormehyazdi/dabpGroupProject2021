...
bob = 100000000000000000000

print(supplies)

lst_5 = []
lst_6 = []
#budget = [5000000, 10000000, 50000000, 75000000]
options = []
...
for b in budget:
    try:
        ...
        for k in blocks:
            ...
            m1.addConstr(sum(cost[i]*x1[i] + z1[i]*opening_cost for i in pod_sites) <= b)
...
for i in pod_sites:
    for k in blocks:
       # print(x1[i].x,i)
       if (y1[i,k].x > 0):
           print("distance:", y1[i,k].x*distance[k,i], (i,k))
...
#Visualization Ideas
# For All Models (1 and 2):
    # Histogram for distance that each census block travels
    # Pareto-efficient frontiers (for different budgets AND different opening costs)
            # --> We'll have two lists for these values
    # Indexes for POD sites open and all census blocks assigned to each POD (so Jeff can visualize in ArcGIS)
